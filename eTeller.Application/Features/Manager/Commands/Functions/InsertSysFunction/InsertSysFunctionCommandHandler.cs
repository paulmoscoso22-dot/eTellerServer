...
namespace eTeller.Application.Features.Manager.Commands.Functions.InsertSysFunction
{
    public class InsertSysFunctionCommandHandler : IRequestHandler<InsertSysFunctionCommand, bool>
    {
        ...
        public async Task<bool> Handle(InsertSysFunctionCommand request, CancellationToken cancellationToken)
        {
            ...
            var result = await _unitOfWork.ManagerRepository.InsertSysFunctionAsync(
                request.TraUser,
                request.TraStation,
                request.FunName,
...
request.Offline,
            ...
            if (!result)
                _logger.LogWarning("Failed to insert function {FunName} for User={TraUser}", request.FunName, request.TraUser);
