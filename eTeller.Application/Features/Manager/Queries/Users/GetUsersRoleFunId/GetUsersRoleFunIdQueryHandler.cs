...
namespace eTeller.Application.Features.Manager.Queries.Users.GetUsersRoleFunId
{
    public class GetUsersRoleFunIdQueryHandler : IRequestHandler<GetUsersRoleFunIdQuery, IEnumerable<UsersRoleFunctionVm>>
    {
        ...
        public async Task<IEnumerable<UsersRoleFunctionVm>> Handle(GetUsersRoleFunIdQuery request, CancellationToken cancellationToken)
        {
            ...
            var items = await _unitOfWork.ManagerRepository.GetUsersRoleByFunIdAsync(request.FunId, cancellationToken);
            if (items == null) {
                _logger.LogWarning("No items found for FunId={FunId}", request.FunId);
                return Enumerable.Empty<UsersRoleFunctionVm>();
            }
