...
namespace eTeller.Application.Contracts.Manager
{
    /// <summary>
    /// Repository for manager-related stored-procedure operations.
    /// Provides query and command methods for sys_* entities.
    /// </summary>
    public interface IManagerRepository : IBaseSimpleRepository<InfoAutorizzazioneUtente>
    {
        // Query: Users / Access
        ...
        Task<IEnumerable<sys_ROLE>> GetAllRoleAsync(CancellationToken cancellationToken = default);

        /// <summary>Get roles not assigned to a given user id.</summary>
        Task<IEnumerable<sys_ROLE>> GetSysRoleNotForUsrIdAsync(string usrId, CancellationToken cancellationToken = default);

        /// <summary>Get users by role id.</summary>
        Task<IEnumerable<eTeller.Domain.Models.StoredProcedure.UserSelectRole>> GetUsersByRoleIdAsync(int roleId, CancellationToken cancellationToken = default);

            /// <summary>Get roles assigned to a given user id.</summary>
            Task<IEnumerable<sys_ROLE>> GetSysRoleByUsrIdAsync(string usrId, CancellationToken cancellationToken = default);

        /// <summary>Get functions by role id.</summary>
        Task<IEnumerable<FunctionRole>> GetFunctionRoleByRoleIdAsync(int roleId, string? funLikeName, string? funLikeDes, CancellationToken cancellationToken = default);
        ...
        /// <summary>Reset user password by user id.</summary>
        Task<int> ResetPasswordAsync(string usrId, bool chgPas, string usrPass, CancellationToken cancellationToken = default);
    }
}
